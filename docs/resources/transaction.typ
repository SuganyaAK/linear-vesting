...
#let tx_out(input, position, inputHeight, styles) = {
  ...
  let value = if "value" in input [
    *Value:* #if ("ada" in input.value) [ *#input.value.ada* ADA ] \
    #v(-1.0em)
    #stack(dir: ttb, spacing: 0.4em, ..input.value.pairs().map(((k, v)) => [
      #if k != "ada" {
        [#h(2.3em) \+ *#v* #raw(k)]
      }
    ]))
      ...
      [
        *#input.name*
        ...
        #place(
          dx: 10em,
          dy: -1.5em,
        )[#circle(radius: 0.5em, fill: white, stroke: black)]
...
#let transaction(
  ...
  styles => {
    ...
    let inputs = [
      ...
      #for input in inputs {
        ...
        place(
          dx: start.x + 10.26em,
          dy: start.y + 0.59em,
        )[#circle(radius: 0.25em, fill: black)]
    ...
    let mints = if show_mints and display_mint.len() > 0 [
      *Mint:* \
    ...
    let transaction = [
      ...
      #rect(
        ...
        [
          ...
          #v(1em)
          #set align(left)
          #stack(dir: ttb, spacing: 1em, mints, sigs, certs, valid_range)
        ],
      )
    ]
