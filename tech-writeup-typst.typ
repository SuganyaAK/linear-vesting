...
This document outlines the design specification for an upgradable multisignature
(multisig) smart contract implemented using Aiken for the Cardano blockchain.
The contract enables authorized members to execute asset transactions within
predefined thresholds, demonstrates secure spending of assets, allows seamless
adjustment of signer thresholds, and supports dynamic addition or removal of
signers for enduring usability and adaptability.
...
- *Signers*
  ...
  Entities who are authorized to sign transactions, participate in the management
  of the multi-sig wallet, and approve changes to the signer list or thresholds.

- *Initiator*

  The entity who creates the initial multi-sig wallet setup, defining the initial
  set of signers and thresholds.
...
=== Multi-sig validator
\
The Multi-sig Contract is the primary contract responsible for managing the list
of authorized signers, validating transactions, and ensuring the proper
execution of multi-sig operations.
...
==== Spend Purpose
\
The contract uses the Spend purpose, allowing it to manage and spend funds
locked in its address.
...
+ *Sign*
  ...
  The redeemer allows a majority of the authorized signers to collectively approve
  and execute transactions using the funds controlled by the multi-signature
  contract

  - Verifies that the required number of authorized signers have signed the
    transaction
  ...
  - Ensure the output datum matches the input datum (no changes to the multisig
    configuration)
...
+ *Update*
  ...
  - Enforce bounds on new signers list and threshold:
    ...
    - New threshold must be greater than 0 and less than or equal to the new signer
      count

    - New funds quantity must be greater than or equal to 0

    - New spending limit must be greater than or equal to 0 and less than or equal to
      the new funds quantity
  ...
  - Verify that the total value is preserved (input value equals output value, no
    funds are spent during update)
...
= Transactions
\
This section outlines the various transactions involved in the Upgradable
Multi-Signature Contract on the Cardano blockchain.\
=== Spend :: Sign
\

This action ensures that the number of signers meets or exceeds the specified
threshold and The datum of the Multisig remains the same.

\
#transaction(
  "lockTokens",
  inputs: ((name: "Wallet UTxO", address: "User"),),
  outputs: (
    (name: "Contract UTxO", value: (ada: "min ADA"), datum: (
      a: "beneficiary",
      b: "vestingAsset",
      c: "totalVestingQty",
      e: "vestingPeriodStart",
      f: "vestingPeriodEnd",
      g: "firstUnlockPossibleAfter",
      h: "totalInstallments",
    )),
    (
      name: "Output UTxO",
      address: "Any User Address",
      value: (ada: "balance ADA"),
      // datum: (a: "signers", b: "threshold", c: "funds", e: "spending_limit"),,,
    ),
  ),
  // signatures: ("Signer A", "Signer B", "Signer C",),
  // // certificates: (
  // //   "Withdraw Stake A",
  // // ),
  show_mints: false,
...
#transaction(
  ...
  inputs: (
    (
      ...
      value: (ada: 100000),
      datum: (a: "old_signers", b: "old_threshold", c: "funds", e: "spending_limit"),
    ),
  ),
  outputs: (
    (
      ...
      address: "multisig_contract",
      value: (ada: 100000),
      datum: (a: "new_signers", b: "new_threshold", c: "funds", e: "spending_limit"),
    ),
  ),
  signatures: ("Signer A", "Signer B", "Signer C",),
  show_mints: false,
